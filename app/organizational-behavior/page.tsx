...
export default function OrganizationalBehaviorPage() {
  return <DashboardShell role="executive"><div className="dashboard-page-stack">
    ...
    <Card className="panel-card"><CardHeader><CardTitle>نقشه وضعیت واحدهای سازمانی</CardTitle><Badge variant="outline">دوره شهریور ۱۴۰۵</Badge></CardHeader><CardContent className="unit-heatmap">{departments.map((department)=><Link href={`/departments/${department.slug}`} className={`page-link heat-card ${department.score<63?"risk":department.score<72?"warn":""}`} key={department.slug}><div className="heat-head"><strong>{department.name}</strong><span className="heat-score">{department.score}</span></div><div className="heat-bar"><b style={{width:`${department.score}%`}} /></div><small>{department.people} نفر · {department.risk} مورد پرریسک</small></Link>)}</CardContent></Card>
    <section className="analysis-grid">
