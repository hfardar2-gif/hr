...
export function OrganizationalBehaviorDashboard({ role }: { role: "executive" | "hr" }) {
  return <div className="dashboard-page-stack">
    ...
    <Card className="panel-card"><CardHeader><CardTitle>نقشه وضعیت واحدهای سازمانی</CardTitle><Badge variant="outline">دوره شهریور ۱۴۰۵</Badge></CardHeader><CardContent className="unit-heatmap">{departments.map((department)=>{ const content=<><div className="heat-head"><strong>{department.name}</strong><span className="heat-score">{department.score}</span></div><div className="heat-bar"><b style={{width:`${department.score}%`}} /></div><small>{department.people} نفر · {department.risk} مورد پرریسک</small></>; const className=`page-link heat-card ${department.score<63?"risk":department.score<72?"warn":""}`; return <Link href={`${role==="hr"?"/hr":""}/departments/${department.slug}`} className={className} key={department.slug}>{content}</Link>; })}</CardContent></Card>
    <section className="analysis-grid">
      <Card className="panel-card"><CardHeader><CardTitle>مدل امتیازدهی نسخه صفر</CardTitle><ShieldCheck /></CardHeader><CardContent><div className="model-weights">{weights.map(([label,value])=><div className="weight-item" key={label}><strong>{value}</strong><span>{label}</span></div>)}</div><p className="risk-reason">وزن‌ها برای نسخه صفر طراحی شده‌اند و پیش از استفاده واقعی باید با مدیران، متخصص منابع انسانی و کنترل سوگیری اعتبارسنجی شوند.</p></CardContent></Card>
