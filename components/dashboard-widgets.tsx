...
export function SectionHeading({ title, description, action }: { title: string; description?: string; action?: string }) {
  return <div className="section-heading"><div><h2>{title}</h2>{description && <p>{description}</p>}</div>{action && <button>{action}<ArrowLeft /></button>}</div>;
}

export function StatCard({ label, value, unit, trend, trendLabel, tone = "navy", icon: Icon }: any) {
  const up = trend > 0;
  return <Card className={`stat-card tone-${tone}`}><CardContent className="p-0"><div className="stat-top"><span className="stat-icon"><Icon /></span><MoreHorizontal /></div><p>{label}</p><div className="stat-value"><strong>{value}</strong>{unit && <span>{unit}</span>}</div><div className={`stat-trend ${up ? "positive" : trend < 0 ? "negative" : "neutral"}`}>{up ? <ArrowUp /> : trend < 0 ? <ArrowDown /> : <Minus />}<span>{Math.abs(trend)}٪ {trendLabel}</span></div></CardContent></Card>;
}
...
export const departments = [
  { name: "مالی", people: 24, score: 89, risk: 1, trend: 6, state: "ممتاز", tone: "excellent" },
  { name: "سرمایه‌های انسانی", people: 18, score: 83, risk: 1, trend: 4, state: "مطلوب", tone: "good" },
  { name: "فروش", people: 32, score: 76, risk: 3, trend: 2, state: "مطلوب", tone: "good" },
  { name: "بازرگانی", people: 26, score: 68, risk: 4, trend: -2, state: "نیازمند توسعه", tone: "develop" },
  { name: "انبار", people: 30, score: 65, risk: 5, trend: -1, state: "نیازمند توسعه", tone: "develop" },
  { name: "نگهداری و تعمیرات", people: 35, score: 59, risk: 8, trend: -5, state: "نیازمند توجه", tone: "attention" },
  { name: "تولید", people: 135, score: 61, risk: 14, trend: -5, state: "نیازمند توجه", tone: "attention" },
];

export function DepartmentTable({ hr = false }: { hr?: boolean }) {
  return <div className="table-wrap"><table className="department-table"><thead><tr><th>واحد سازمانی</th><th>تعداد نیرو</th><th>امتیاز</th><th>موارد پرریسک</th><th>روند</th><th>وضعیت</th>{hr && <th>اقدام</th>}</tr></thead><tbody>{departments.map((d) => <tr key={d.name}><td><span className="dept-avatar">{d.name.slice(0, 1)}</span><strong>{d.name}</strong></td><td>{d.people} نفر</td><td><div className="score-cell"><span>{d.score}</span><i><b style={{ width: `${d.score}%` }} /></i></div></td><td><strong className={d.risk > 5 ? "risk-number" : ""}>{d.risk}</strong></td><td><span className={d.trend > 0 ? "trend-up" : "trend-down"}>{d.trend > 0 ? <ArrowUp /> : <ArrowDown />}{Math.abs(d.trend)}٪</span></td><td><Badge className={`status-badge ${d.tone}`}>{d.state}</Badge></td>{hr && <td><button className="row-action">بررسی <ArrowLeft /></button></td>}</tr>)}</tbody></table></div>;
}
