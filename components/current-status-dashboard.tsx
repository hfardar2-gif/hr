...
function UnitAxisTick({x=0,y=0,payload}:{x?:number;y?:number;payload?:{value?:string}}) {
  return <text x={x-10} y={y} dy="0.35em" fill="#ffffff" fontSize={12} fontWeight={700} textAnchor="end" direction="rtl" style={{fontFamily:"Vazirmatn, Tahoma, Arial, sans-serif"}}>{payload?.value??""}</text>;
}
    ...
    <div className="live-tab-content">
        ...
        <section className="live-section"><div className="live-section-title"><div><span>۱</span><h2>ساختار جمعیتی سازمان</h2></div><p>تصویر ایستای ترکیب نیروی انسانی در بازه انتخاب‌شده</p></div>
          <div className="live-chart-grid">
            ...
            <ChartBox title="توزیع پرسنل در واحدها" subtitle="مقایسه ظرفیت نیروی انسانی" wide><ResponsiveContainer width="100%" height={310}><BarChart data={visibleUnits} layout="vertical" margin={{left:25}}><CartesianGrid strokeDasharray="3 3" horizontal={false}/><XAxis type="number"/><YAxis type="category" dataKey="name" width={115} tick={<UnitAxisTick/>} tickLine={false}/><Tooltip contentStyle={tooltipStyle}/><Bar dataKey="count" name="نفر" fill="#17375e" radius={[0,8,8,0]}/></BarChart></ResponsiveContainer></ChartBox>
        ...
        <section className="live-section"><div className="live-section-title"><div><span>۲</span><h2>روندها و هشدارهای مدیریتی</h2></div><p>تحلیل تغییرات ۱۲ ماه اخیر و نقاط نیازمند اقدام</p></div>
          <div className="live-chart-grid">
            ...
            <ChartBox title="نرخ ترک خدمت واحدها" subtitle="بالاتر از ۲۰٪ با رنگ قرمز"><ResponsiveContainer width="100%" height={300}><BarChart data={visibleUnits} layout="vertical"><CartesianGrid strokeDasharray="3 3" horizontal={false}/><XAxis type="number" unit="%"/><YAxis type="category" dataKey="name" width={110} tick={<UnitAxisTick/>} tickLine={false}/><Tooltip contentStyle={tooltipStyle}/><Bar dataKey="attrition" name="نرخ ترک خدمت" radius={[0,7,7,0]}>{visibleUnits.map(item=><Cell key={item.name} fill={item.attrition>20?"#dc4c55":item.attrition>12?"#e99a31":"#2b9f93"}/>)}</Bar></BarChart></ResponsiveContainer></ChartBox>
            <ChartBox title="نرخ غیبت به تفکیک واحد" subtitle="مرتب‌شده بر اساس میانگین روز غیبت"><ResponsiveContainer width="100%" height={300}><BarChart data={[...visibleUnits].sort((a,b)=>b.absence-a.absence)} layout="vertical"><CartesianGrid strokeDasharray="3 3" horizontal={false}/><XAxis type="number"/><YAxis type="category" dataKey="name" width={110} tick={<UnitAxisTick/>} tickLine={false}/><Tooltip contentStyle={tooltipStyle}/><Bar dataKey="absence" name="روز غیبت" fill="#586f9f" radius={[0,7,7,0]}/></BarChart></ResponsiveContainer></ChartBox>
          </div>
